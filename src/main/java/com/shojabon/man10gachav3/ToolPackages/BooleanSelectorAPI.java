...
public class BooleanSelectorAPI {
    ...
    public BooleanSelectorAPI(String title, Player p, ItemStack itemToDisplay, boolean current, BiFunction<InventoryClickEvent, Boolean, String> biFunction, Function<InventoryClickEvent, String> backFunction){
        ...
        inv.fillInventory(new SItemStack(Material.BLUE_STAINED_GLASS_PANE).setDisplayname(" ").build());
        inv.setItem(31, new SItemStack(Material.CHEST_MINECART).setDisplayname("§b§n§l変更を保存する").build());
    ...
    private void render(boolean b){
        ItemStack trueGlass = new SItemStack(Material.LIME_STAINED_GLASS_PANE).setDisplayname("§a§lTrue").build();
        ItemStack falseGlass = new SItemStack(Material.RED_STAINED_GLASS_PANE).setDisplayname("§c§lFalse").build();
        if(b){
