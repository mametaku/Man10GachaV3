...
public class ItemStackSelectorAPI {
    ...
    public ItemStackSelectorAPI(String title, Player p, ItemStack currentItem, int amount, BiFunction<InventoryClickEvent, ItemStack, String> okFunction, Function<InventoryClickEvent, String> cancelFunction){
        ...
        inv = new SInventory(3, title).fillInventory(new SItemStack(Material.BLUE_STAINED_GLASS_PANE).setDisplayname(" ").build()).
                setItem(new int[]{11, 12}, new SItemStack(Material.LIME_STAINED_GLASS_PANE).setDisplayname("§a§l決定").build()).
                setItem(new int[]{14, 15}, new SItemStack(Material.RED_STAINED_GLASS_PANE).setDisplayname("§c§lキャンセル").build()).setItem(13, new SItemStack(currentItem).setAmount(amount).build()).
