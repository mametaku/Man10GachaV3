...
public class TitleTextSelectorAPI {
    ...
    public TitleTextSelectorAPI(Player p, String title, GachaTitleText titleText, BiFunction<InventoryClickEvent, GachaTitleText, String> okFucntion, Function<InventoryClickEvent, String> cancelFunction){
        ...
        inventory.fillInventory(new SItemStack(Material.BLUE_STAINED_GLASS_PANE).setDisplayname(" ").build());
        inventory.setItem(new int[]{40, 41, 42}, new SItemStack(Material.RED_STAINED_GLASS_PANE).setDisplayname("§c§l決定").build());
        inv = inventory.build();
    ...
    private void render(GachaTitleText titleText){
        inv.setItem(10, new SItemStack(Material.SPRUCE_SIGN).setDisplayname("§b§lメインタイトル").addLore("§b§l現在設定：" + titleText.getMainText()).build());
        inv.setItem(28, new SItemStack(Material.SPRUCE_SIGN).setDisplayname("§b§lサブテキスト").addLore("§b§l現在設定：" + titleText.getSubText()).build());
        inv.setItem(19, new SItemStack(Material.BARRIER).setDisplayname("§b§lタイトル設定をクリアする").build());
        inv.setItem(21, new SItemStack(Material.CLOCK).setDisplayname("§b§lフェードイン時間設定").addLore("§b§l現在設定：" + titleText.getFadeinTime()).build());
        inv.setItem(23, new SItemStack(Material.CLOCK).setDisplayname("§b§l表示時間設定").addLore("§b§l現在設定：" + titleText.getTime()). build());
        inv.setItem(25, new SItemStack(Material.CLOCK).setDisplayname("§b§lフェードアウト時間設定").addLore("§b§l現在設定：" + titleText.getFadeoutTime()).build());
    }
