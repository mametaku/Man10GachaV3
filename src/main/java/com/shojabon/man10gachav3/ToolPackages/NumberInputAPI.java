...
public class NumberInputAPI {
    ...
    public NumberInputAPI(String title, Player player, int numberOfChars, BiFunction<InventoryClickEvent, Integer, String> function, Function<InventoryClickEvent, String> backFunction){
        ...
        SInventory inv = new SInventory(6, title);
                inv.fillInventory(new SItemStack(Material.BLUE_STAINED_GLASS_PANE).setDisplayname(" ").build()).setItem(new int[]{41, 42,50,51}, new SItemStack(Material.EMERALD_BLOCK).setDisplayname("§a§l§nConfirm").build()).
        ...
        for(int i = 0;i < redTilesToPlace;i++){
            inv.setItem(i, new SItemStack(Material.RED_STAINED_GLASS_PANE).setDisplayname(" ").build());
        }
        inv.setItem(new int[]{12, 15}, new SItemStack(Material.BLUE_STAINED_GLASS_PANE).setGlowingEffect(true).setDisplayname("§a§l,").build());
